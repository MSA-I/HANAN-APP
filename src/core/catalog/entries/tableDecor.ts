...
export const tableDecorEntries: CatalogEntry[] = [
  ...
  // goes to the image model, so the distinction is the difference between a cuff of
  // a different material and a twist of the same cloth.
  //
  // Copper linen, and NINE marked parts rather than one — body, ring and seven
  // fragments of it, every one of them 0.41…0.80 away from the charger's grey. They
  // all take the ONE slot: the user was asked and said the ring may share the
  // napkin's colour, which is what makes the most fragmented of the five the
  // simplest to wire.
  cover('decor.place-setting-tied', 'decorPlaceSettingTied', 'a full place setting with a napkin gathered upright through a napkin ring: charger, plate, cutlery and two glasses', 'decor-place-setting-tied.glb', { width: 36, depth: 32.64, height: 14.88 }, { width: 45, depth: 40.8, height: 18.6 }, '#7d5d49'),
]
