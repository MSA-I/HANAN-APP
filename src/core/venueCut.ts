...
/** plan cm bbox of a triangle set, or null for an empty one. */
export function boundsOf(tris: readonly CutTriangle[]): CutBounds | null {
  if (!tris.length) return null
  const b = { minX: Infinity, minY: Infinity, maxX: -Infinity, maxY: -Infinity }
  for (const t of tris) {
    for (let k = 0; k < 6; k += 2) {
      b.minX = Math.min(b.minX, t[k])
      b.maxX = Math.max(b.maxX, t[k])
      b.minY = Math.min(b.minY, t[k + 1])
      b.maxY = Math.max(b.maxY, t[k + 1])
    }
  }
  return b
}
...
/**
 * Split the walls by which side of the venue they enclose.
 *
 * The hall and the reception deck are two separate buildings sharing one sheet,
 * and the editor shows one at a time — so the walls have to take sides as well,
 * or the deck's envelope goes on being drawn after everything inside it has gone.
 *
 * The test is the triangle's CENTROID, which lies in exactly one of the two. Its
 * corners do not: a wall along the deck's boundary has corners either side of it.
 */
export function splitWalls(
  tris: readonly CutTriangle[],
  zone: { x: number; y: number; width: number; depth: number },
): { inside: CutTriangle[]; outside: CutTriangle[] } {
  const inside: CutTriangle[] = []
  const outside: CutTriangle[] = []
  for (const t of tris) {
    const cx = (t[0] + t[2] + t[4]) / 3
    const cy = (t[1] + t[3] + t[5]) / 3
    const within =
      cx >= zone.x && cx <= zone.x + zone.width && cy >= zone.y && cy <= zone.y + zone.depth
    ;(within ? inside : outside).push(t)
  }
  return { inside, outside }
}

/** The side of the walls a view is showing: `hallSide` picks the hall's, else the zone's. */
export function pickWalls(
  tris: readonly CutTriangle[],
  zone: { x: number; y: number; width: number; depth: number },
  hallSide: boolean,
): CutTriangle[] {
  const { inside, outside } = splitWalls(tris, zone)
  return hallSide ? outside : inside
}
